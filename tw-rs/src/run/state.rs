...
impl CiState {
    pub fn new() -> anyhow::Result<Self> {
        ...
        if is_dirty {
            eprintln!(
                "[{}] WARNING: LOCAL_CI repo is dirty, appending -dirty to commit tasks",
                Utc::now().format("%Y-%m-%d %H:%M:%S")
            );
        ...
        let nix_files = cmd!(
            sh,
            "find . -maxdepth 2 ( -name '*.nix' -o -name '*.patch' ) -type f"
        )
        .read()
        ...
        for nix_file in nix_files.lines() {
            if !nix_file.trim().is_empty() {
                ...
                let dst = temp_nix_dir
                    .path()
                    .join(nix_file.trim().trim_start_matches("./"));
        ...
        eprintln!(
            "[{}] Copied .check-pr.nix files to temp directory: {}",
            Utc::now().format("%Y-%m-%d %H:%M:%S"),
    ...
    pub fn check_ci_repo_status(&mut self) -> anyhow::Result<()> {
        ...
        if current_commit != expected_commit {
            eprintln!(
                "[{}] WARNING: LOCAL_CI commit ID changed from {} to {}. Please restart the program.",
                Utc::now().format("%Y-%m-%d %H:%M:%S"),
                expected_commit,
                current_commit
            );
        ...
        if is_dirty && !already_dirty {
            eprintln!(
                "[{}] WARNING: LOCAL_CI repo became dirty. Please restart the program.",
                Utc::now().format("%Y-%m-%d %H:%M:%S")
            );
        } else if !is_dirty && already_dirty {
            eprintln!(
                "[{}] WARNING: LOCAL_CI repo is no longer dirty but was dirty on startup. Please restart the program.",
                Utc::now().format("%Y-%m-%d %H:%M:%S")
            );
        }
